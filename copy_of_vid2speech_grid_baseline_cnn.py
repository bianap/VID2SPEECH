# -*- coding: utf-8 -*-
"""Copy of VID2SPEECH_GRID_baseline_CNN.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1BNuyYaqcDr5FCknYiRIlhAzNo5y-94m5
...
samples = np.zeros((lip.shape[0]), dtype=[('input', float, (128, 128)), ('output', float, 13)])
for i in range(0, len(lip)):
    samples[i] = lip[i].reshape(n_lines, n_pixels), mgc[i]
print(samples[0]['input'])

train_samples = np.zeros(int(0.9*lip_size), dtype=[('input', float, (128, 128)), ('output', float, 13)])
valid_samples = np.zeros(int(0.1*lip_size), dtype=[('input', float, (128, 128)), ('output', float, 13)])
...
train_x = np.reshape(train_samples['input'], (len(train_samples), 128, 128, 1))
valid_x = np.reshape(valid_samples['input'], (len(valid_samples), 128, 128, 1))

from keras.layers import Conv2D, MaxPooling2D, Flatten, Dropout
model = Sequential()
model.add(Conv2D(input_shape=(128, 128, 1), filters=32, kernel_size=(3,3), strides=(1, 1), padding='same', activation="tanh", use_bias=True))
model.add(Conv2D(filters=32, kernel_size=(3,3), strides=(1, 1), padding='same', activation="tanh", use_bias=True))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Conv2D(filters=64, kernel_size=(3,3), strides=(1, 1), padding='same', activation="tanh", use_bias=True))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Flatten())
...
# Run training
history = model.fit(train_x, train_samples['output'],
...
if os.path.isdir(dir_video):
    for file in sorted(os.listdir(dir_video)):
        if ".avi" in file and n_file < n_files and "00990"  in file:
...
display.specshow(logH_original.T, sr=8000, hop_length=frameShift, x_axis="time", y_axis="linear")
